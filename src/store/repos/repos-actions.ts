import { NormalizedResponseDTO, Repository } from '@kyso-io/kyso-model';
...
export const fetchReposAction = createAsyncThunk('repos/fetchRepos', async (_, { getState }) => {
  try {
    ...
    const axiosResponse: AxiosResponse<NormalizedResponseDTO<Repository[]>> = await httpClient.get(url);
...
export const fetchRepoTreeAction = createAsyncThunk('repos/fetchTree', async (payload: { owner: string; branch: string }, { getState }) => {
  try {
    ...
    const axiosResponse: AxiosResponse<NormalizedResponseDTO<object>> = await httpClient.get(url);
    if (axiosResponse?.data?.data) {
      return axiosResponse.data.data;
    } else {
      return null;
