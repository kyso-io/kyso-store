...
export const fetchUserPortraitUrl = createAsyncThunk(
  'user/fetchUserPortraitUrl',
  async (args: { userId: string }, { getState, dispatch }): Promise<string | null> => {
    try {
      const url = `${getAPIBaseURL()}/users/${args.userId}/portrait`;
...
export const fetchUserProfileActionByUserId = createAsyncThunk(
  'user/fetchUserProfileByUserId',
  async (args: { userId: string }, { getState, dispatch }): Promise<UserDTO | null> => {
    try {
